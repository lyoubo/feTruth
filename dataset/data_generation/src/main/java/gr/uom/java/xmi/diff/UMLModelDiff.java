...
import org.refactoringminer.api.MappedElement;
...
public class UMLModelDiff {
    ...
    private void detectSubRefactorings(UMLClassBaseDiff classDiff, UMLClass addedClass, RefactoringType parentType) throws RefactoringMinerTimedOutException {
        for (UMLOperation addedOperation : addedClass.getOperations()) {
            ...
            if (removedOperation != null) {
                ...
                if (ref != null) {
                    ref.setFilterType("subRefactoring");
                    this.refactorings.add(ref);
    ...
    public List<Refactoring> getMoveMethodRefactorings() throws RefactoringMinerTimedOutException {
        Set<Refactoring> refactorings = new LinkedHashSet<Refactoring>();
        checkForOperationMovesBetweenCommonClasses();
        refactorings.addAll(this.refactorings);
        return filterOutDuplicateRefactorings(refactorings);
                ...
                if (!operationBodyMapperMap.isEmpty()) {
                    ...
                    for (UMLOperationBodyMapper firstMapper : firstMappers) {
                        ...
                        if (refactoring != null) {
                            ...
                            refactoring.setMappings(mappedElements(firstMapper));
                            refactorings.add(refactoring);
    ...
    public MappedElement mappedElements(UMLOperationBodyMapper operationBodyMapper) {
        int mappedElements = operationBodyMapper.mappingsWithoutBlocks();
        int nonMappedElementsT1 = operationBodyMapper.nonMappedElementsT1();
        int nonMappedElementsT2 = operationBodyMapper.nonMappedElementsT2();
        ...
        int sourceLOE = mappedElements + nonMappedElementsT1;
        int targetLOE = mappedElements + nonMappedElementsT2;
        int mappedLOE = mappedElements + nonMappedStatementsDeclaringSameVariable + nonMappedLoopsIteratingOverSameVariable;
        mappedElement.setMappedLOE(mappedLOE);
        mappedElement.setSourceLOE(sourceLOE);
        mappedElement.setTargetLOE(targetLOE);
